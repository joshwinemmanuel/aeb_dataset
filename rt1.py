...
def process_md5(md5_data):
    ...
    return dict(
        ...
        latitude=latitude,
        longitude=longitude,
        country=country_code,  # Add country code
        Mviz=Mviz
    )

def save_logs(new_data, filename, include_full_data=True):
    ...
    with open(filename, 'a', newline='') as f:
        ...
        if not file_exists:
            if include_full_data:
                writer.writerow(['MD5', 'Location', 'Times of Day', 'Weather', 'Latitude', 'Longitude', 'Country', 'Mviz'])
            else:
                writer.writerow(['MD5', 'Latitude', 'Longitude', 'Country'])

        for data in new_data:
            if include_full_data:
                writer.writerow([data['MD5'], data['location'], data['times_of_day'], data['weather'], data['latitude'], data['longitude'], data['country'], data['Mviz']])
            else:
                writer.writerow([data['MD5'], data['latitude'], data['longitude'], data['country']])
                ...
                if md5_data:
                    ...
                    if latitude is not None and longitude is not None:
                        if is_coordinate_on_land(latitude, longitude, land_gdf):
                            ...
                            # Directly log MD5 with invalid (non-land) coordinates
                            save_logs([[md5, latitude, longitude, None]], 'invalid_coordinates_log.csv', include_full_data=False)
                    else:
